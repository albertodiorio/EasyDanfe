...
internal class DanfePagina
{
    ...
    public DanfePagina(Danfe danfe)
    {
        ...
        if (Danfe.ViewModel.Orientacao == Orientacao.Retrato)
            Retangulo = new RectangleF(0, 0, Constants.Constants.A4Largura, Constants.Constants.A4Altura);
        else
            Retangulo = new RectangleF(0, 0, Constants.Constants.A4Altura, Constants.Constants.A4Largura);
    ...
    {
        var ts = new TextStackElement(RetanguloCorpo) { AlinhamentoVertical = AlinhamentoVertical.Centro, AlinhamentoHorizontal = AlinhamentoHorizontal.Centro, LineHeightScale = 0.9F }
    ...
    public void DesenharBlocos(bool isPrimeirapagina = false)
    {
        ...
        var blocos = isPrimeirapagina ? Danfe.Modules : Danfe.Modules.Where(x => x.VisivelSomentePrimeiraPagina == false);
        ...
        foreach (var bloco in blocos)
        {
            ...
            if (bloco is ModuleIdentificacaoEmitente)
            {
                var rf = (bloco as ModuleIdentificacaoEmitente).RetanguloNumeroFolhas;
