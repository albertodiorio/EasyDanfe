...
namespace EasyDanfe.Graphics;

internal class Gfx(PrimitiveComposer primitiveComposer)
{
    public PrimitiveComposer PrimitiveComposer { get; private set; } = primitiveComposer ?? throw new ArgumentNullException(nameof(primitiveComposer));
    ...
    {
        ArgumentNullException.ThrowIfNull(xobj);
        CheckRectangle(r);
    ...
    private static void CheckRectangle(RectangleF r)
    {
        if (r.X < 0 || r.Y < 0 || r.Width <= 0 || r.Height <= 0) throw new ArgumentException(nameof(r));
    }

    private static void CheckPoint(PointF p)
    {
