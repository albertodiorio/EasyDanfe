...
public class DanfeModel
{
    ...
    public virtual string TextoAdicional()
    {
        ...
        if (!string.IsNullOrEmpty(Pedido) && !Utils.Utils.StringContemChaveValor(InformacoesComplementares, "Pedido", Pedido))
            sb.AppendChaveValor("Pedido", Pedido);

        if (!string.IsNullOrEmpty(Contrato) && !Utils.Utils.StringContemChaveValor(InformacoesComplementares, "Contrato", Contrato))
