...
{
    public readonly static IEnumerable<FormaEmissao> FormasEmissaoSuportadas = [FormaEmissao.Normal, FormaEmissao.ContingenciaSVCAN, FormaEmissao.ContingenciaSVCRS];
    ...
    private static EmpresaViewModel CreateEmpresaFrom(Empresa empresa)
    {
        EmpresaViewModel model = new()
        {
            ...
            CnpjCpf = !string.IsNullOrWhiteSpace(empresa.CNPJ) ? empresa.CNPJ : empresa.CPF,
            Ie = empresa.IE,
            IeSt = empresa.IEST,
            Email = empresa.email
        };
    ...
    {
        ProcNFe nfe = null;
        XmlSerializer serializer = new(typeof(ProcNFe));
    ...
    {
        using StreamReader sr = new(caminho, true);
    ...
    public static DanfeViewModel CreateFromXml(ProcNFe procNfe)
    {
        ...
        model.ProtocoloAutorizacao = string.Format(Utils.Formatador.Cultura, "{0} - {1}", infoProto.nProt, infoProto.dhRecbto.DateTimeOffsetValue.DateTime);
