...
internal class TabelaElement(EstiloElement estilo) : ElementBase(estilo)
{
    public List<TabelaColunaElement> Colunas { get; private set; } = [];
    public List<List<string>> Linhas { get; private set; } = [];
    public float PaddingSuperior { get; private set; } = 0.75F;
    public float PaddingInferior { get; private set; } = 0.3F;
    public float PaddingHorizontal { get; private set; } = 0.6F;

    public int LinhaAtual { get; private set; } = 0;
    public float TamanhoFonteCabecalho { get; private set; } = 6;

    private float _DY;
    private float _DY1;

    public Fonte FonteCorpo { get; private set; } = estilo.CriarFonteRegular(6F);
    public Fonte FonteCabecalho { get; private set; } = estilo.CriarFonteRegular(6F);

    public TabelaElement ComColuna(float larguraP, AlinhamentoHorizontal ah, params String[] cabecalho)
    {
        Colunas.Add(new TabelaColunaElement(cabecalho, larguraP, ah));
        return this;
    }
    ...
    public void AjustarLarguraColunas()
    {
        ...
        var w = (100F - sw) / Colunas.Where(x => x.PorcentagemLargura == 0).Count();
    ...
    private bool DesenharLinha(Gfx gfx)
    {
        ...
        var tb = new TextBlockElement[Colunas.Count];
        ...
        for (int i = 0; i < Colunas.Count; i++)
        {
            ...
            if (!string.IsNullOrWhiteSpace(v))
            {
                ...
                tb[i] = new TextBlockElement(v, FonteCorpo)
                {
    ...
    public void DesenharCabecalho(Gfx gfx)
    {
        ...
        foreach (var coluna in Colunas)
        {
            ...
            var tb = new TextStackElement(r.InflatedRetangle(1F));
        ...
        while (LinhaAtual < Linhas.Count)
        {
            ...
            if (r)
            {
                if (LinhaAtual > 0)
                {
                    ...
                    gfx.PrimitiveComposer.SetLineDash(new org.pdfclown.documents.contents.LineDash([6, 1]));
